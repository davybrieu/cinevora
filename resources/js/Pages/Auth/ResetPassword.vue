    ...
    <AuthLayout :title="t('reset_password')">
        <div class="relative z-10 flex flex-1 flex-col items-center justify-center px-8">
            <Card class="w-full max-w-md">
                <h1 class="mb-3 text-2xl font-bold text-white">{{ t('reset_password') }}</h1>
                <p class="mb-6 text-sm text-white/60">{{ t('reset_password_hint') }}</p>
                ...
                <form @submit.prevent="submit" class="space-y-5">
                    ...
                    <InputGroup id="password_confirmation" v-model="form.password_confirmation"
                        :label="t('confirm_password')" type="password" :placeholder="t('placeholder_confirm_password')"
